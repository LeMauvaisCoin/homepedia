...
/** Resolve interfaces through the same lexical bindings used for aliases. */
export function visibleTypeInterfaces(
	...
	const declarations = nearestTypeBindings(name, use, environment).map(
		(binding) => binding.declaration,
	);
	return declarations.every((node) => node.type === "TSInterfaceDeclaration")
		? declarations.filter(
				(node): node is ESTree.TSInterfaceDeclaration => node.type === "TSInterfaceDeclaration",
			)
		: [];
}
