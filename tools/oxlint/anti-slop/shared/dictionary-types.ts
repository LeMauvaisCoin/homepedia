...
export function isFiniteProjection(type: ESTree.TSType, environment: TypeEnvironment): boolean {
	...
	const domains = broadKeyDomains(key, environment, new Map());
	return name === "Pick" ? domains === 0 : domains === 7;
}
...
function aliasSubstitution(
	...
	for (const [index, parameter] of parameters.entries()) {
		...
		next.set(parameter.name.name, resolvedSubstitutionArgument(
			argument, explicitArgument === undefined ? next : base,
		));
	}
	return next;
}
...
function unsafeDirectValue(
	...
	const interfaceDeclarations = visibleTypeInterfaces(
		name, unwrapped, environment.typeAliases,
	);
...
function dictionaryValueTypes(
	...
	if (name === "Record" && isBuiltIn(name, unwrapped, environment)) {
		...
		return value === null || !hasBroadRecordKey(unwrapped, environment, substitutions)
			? []
	...
	if (
		...
		const key = unwrapped.typeArguments?.params[1];
		if (key !== undefined) {
			const domains = broadKeyDomains(key, environment, substitutions);
			if ((name === "Pick" && domains === 0) || (name === "Omit" && domains === 7))
				return [];
		}
...
function isBroadMappedKey(
	...
	return broadKeyDomains(type, environment, substitutions) !== 0;
}

// Bit flags for unbounded string, number, and symbol key domains.
function broadKeyDomains(
	type: ESTree.TSType,
	environment: TypeEnvironment,
	substitutions: TypeAliasEnvironment,
	...
	const unwrapped = unwrapTransparentType(type);
	if (unwrapped.type === "TSStringKeyword") return 1;
	if (unwrapped.type === "TSNumberKeyword") return 2;
	if (unwrapped.type === "TSSymbolKeyword") return 4;
	if (unwrapped.type === "TSAnyKeyword" || unwrapped.type === "TSUnknownKeyword") return 7;
	if (unwrapped.type === "TSUnionType") {
		return unwrapped.types.reduce((domains, member) =>
			domains | broadKeyDomains(member, environment, substitutions, visitedAliases), 0);
	}
	if (unwrapped.type === "TSIntersectionType") {
		return unwrapped.types.reduce((domains, member) => {
			// Object brands do not narrow a primitive domain to a finite set of keys.
			if (unwrapTransparentType(member).type === "TSTypeLiteral") return domains;
			return domains & broadKeyDomains(member, environment, substitutions, visitedAliases);
		}, 7);
	}
	if (unwrapped.type !== "TSTypeReference") return 0;
	const name = typeReferenceName(unwrapped);
	if (name === null || visitedAliases.has(name)) return 0;
	const nextVisited = new Set(visitedAliases);
	nextVisited.add(name);
	const substitution = substitutions.get(name);
	if (substitution !== undefined && !isUnappliedReferenceTo(substitution, name)) {
		return broadKeyDomains(substitution, environment, substitutions, nextVisited);
	}
	if (name === "PropertyKey" && isBuiltIn(name, unwrapped, environment)) return 7;
	const alias = visibleTypeAlias(name, unwrapped, environment.typeAliases);
	if (alias === null) return 0;
	const nextSubstitutions = aliasSubstitution(alias, unwrapped, substitutions);
	if (nextSubstitutions === null) return 0;
	return broadKeyDomains(alias.typeAnnotation, environment, nextSubstitutions, nextVisited);
}
...
export function isKnownEvidenceExpression(expression: ESTree.Expression): boolean {
	...
	if (current.type === "ObjectExpression") {
		return !current.properties.some((property) => property.type === "SpreadElement");
	}
	return (
